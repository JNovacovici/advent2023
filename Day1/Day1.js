...
need to recover. On each line, the calibration value can be found by combining the first digit and the last digit (in that order) to form a single two-digit number.
...
const firstandLastDigit = (file) => {
  const lines = fs.readFileSync(file, 'utf-8').trim().split('\n');
  const values = lines.map((text) => {
    let textArray = text.split('');
    let firstDigit = textArray.find((v) => !Number.isNaN(Number(v)));
    let lastDigit = textArray.findLast((v) => !Number.isNaN(Number(v)));
...
const replaceNumberWords = (text) => {
  for (let word of Object.keys(numberWords)) {
    text = text.replaceAll(word, numberWords[word]);
  }
  // console.log(text);
  return text;
}
...
console.log('part 1: ', + firstandLastDigit('./Day1Values.txt'));
console.log('part 2: ', + part2('./Day1Values.txt'));