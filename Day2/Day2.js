...
situation, but it's a bit of a walk, so you have some time. They don't get many visitors up here; would you like to play a game in the meantime?

As you walk, the Elf shows you a small bag and some cubes which are either red, green, or blue. Each time you play this
game, he will hide a secret number of cubes of each color in the bag, and your goal is to figure out information about the number of cubes.
...
(like the 11 in Game 11: ...) followed by a semicolon-separated list of subsets of cubes that were revealed from the bag (like 3 red, 5 green, 4 blue).
...
const CubeGame = (file) => {
  ...
  const checkCubes = (gameData) => {
    const games = gameData.split(';');

    return games.every(subGame => {
      const cubes = subGame.trim().split(', ');
      ...
      return cubes.every(cube => {
        ...
        return !(counts.red > targetCubes.red || counts.green > targetCubes.green || counts.blue > targetCubes.blue)
      });
    });
  };
  ...
  let sumOfGames = 0;
  games.forEach(game => {
    const [id, cubeData] = game.split(':');
    if (checkCubes(cubeData)) {
      sumOfGames += parseInt(id.trim().split(' ')[1]);
    }
  });
...
console.log('part 1: ', + CubeGame('./Day2Values.txt'));